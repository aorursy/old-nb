...
p_high, p_medium
def cardinal(train_df_tr, test_df, f):
    ...
    train_df_tr=train_df_tr.merge(temp2[[f+'_high', f+'_medium']], how='left', 

                                  left_on=[f], right_index=True)

    test_df=test_df.reset_index().merge(temp2[[f+'_high', f+'_medium']], how='left', 

                                  left_on=[f], right_index=True).set_index('index')

    test_df.loc[test_df[f+'_high'].isnull(), f+'_high'] = -1

    test_df.loc[test_df[f+'_medium'].isnull(), f+'_medium'] = -1
...
features_to_use2=features_to_use# + ['street_address_high', 'street_address_medium']
...
(tr, te)=cardinal(tr, te, 'street_address')
