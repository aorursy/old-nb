...
for fl in list(train_df["features"]):

    c.update(fl)

most_common_features = c.most_common()[:30]
most_common_features_featnames = []

for cf in most_common_features:

    cf_colname = cf[0].replace(" ", "_").replace("-", "_")

    featname = "has_feature_"+cf_colname

    train_df[featname] = train_df.apply(

        lambda r: int(cf[0] in r.get('features', [])),

        axis=1

    )

    most_common_features_featnames.append(featname)

train_df[['features']+most_common_features_featnames]
