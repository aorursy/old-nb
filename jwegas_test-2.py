...
# Привести значение атрибутов к соответствующему виду согласно тому, какой тип поля имеет атрибут
#df_feautures['value'] = df_attr['value']
#df_feautures['value'] = df_feautures['value'].apply(lambda x: x.lower() if isinstance(x, basestring) else x)

df_feautures['value'] = df_feautures[['value', 'IsDimension', 'IsWeight']].apply(lambda x: 
                                                         #re.findall(r'[-+]?([0-9]*[\.][0-9]+|[0-9]+)',x[0])
                                                         re.sub(r'[^0-9\.\\ ]', ' ', re.sub(r'\,','.',x[0]))
                                                         if (x[1] == 1 or x[2] == 1) and isinstance(x[0], str)                                                             
                                                         else x[0]
                                                            , axis=1)
df_feautures.head()