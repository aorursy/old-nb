...
for f in ["X0", "X1", "X2", "X3", "X4", "X5", "X6", "X8"]:
        ...
        lbl.fit(list(train_df[f].values + test_df[f].values) )

        yuanlai = list(train_df[f].values + test_df[f].values) 

        train_df[f] = lbl.transform(yuanlai)

        test_df[f] = lbl.transform(yuanlai)
...
model = ensemble.GradientBoostingRegressor(n_estimators=200, max_depth=10, min_samples_leaf=4, max_features=0.2, random_state=0)
...
output.to_csv('myGBDT.csv', index=False)