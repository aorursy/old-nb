...
def historical_transactions(num_rows=None):
    ...
    hist_df['purchase_amount_adjust'] = np.round(hist_df['purchase_amount'] / 0.00150265118 + 497.06,2)
    ...
    aggs['purchase_amount_adjust'] = ['sum','max','min','mean','var','skew']
...
def new_merchant_transactions(num_rows=None):
    ...
    new_merchant_df['purchase_amount_adjust'] = np.round(new_merchant_df['purchase_amount'] / 0.00150265118 + 497.06,2)
...
def kfold_lightgbm(feats, train_df_all, all_preds,sub_preds, train_df, test_df, num_folds, stratified = False, debug= False):
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(folds.split(train_df[feats], train_df['outliers'])):
        ...
        sub_preds += reg.predict(test_df[feats], num_iteration=reg.best_iteration) / (folds.n_splits * n_epoch) 

        all_preds += reg.predict(train_df_all[feats], num_iteration=reg.best_iteration) / (folds.n_splits * n_epoch)
    ...
    train_df_all['predict_proba_outliers'] = all_preds

    test_df['predict_proba_outliers'] = sub_preds

    print('all roc_auc_score : %.6f' % (roc_auc_score(train_df_all['outliers'], all_preds)))
...
def main(debug=False):
    ...
    with timer("Run LightGBM with kfold"):
        ...
        all_preds = np.zeros(train_df_all.shape[0])
        ...
        for i in range(n_epoch):
            ...
            outliers = train_df_all.query('outliers == 1')

            not_outliers = train_df_all.query('outliers == 0').sample(len(outliers))

            train_df = pd.concat([outliers, not_outliers])

            

            kfold_lightgbm(feats, train_df_all, all_preds, sub_preds,

                           train_df, test_df, num_folds=num_folds, stratified=False, debug=debug)

            

        return train_df_all,test_df
train_df_all,test_df = main(False)
train_df_all.reset_index()[['card_id', 'predict_proba_outliers']].to_csv('train_predict_proba_outliers.csv')

test_df.reset_index()[['card_id', 'predict_proba_outliers']].to_csv('test_predict_proba_outliers.csv')
train_df_all.predict_proba_outliers.describe()
train_df_all['predict_outliers']=[1 if p > 0.52 else 0  for p in train_df_all.predict_proba_outliers.values]
...
confusion_matrix(train_df_all.outliers, train_df_all.predict_outliers)
accuracy_score(train_df_all.outliers, train_df_all.predict_outliers)
f1_score(train_df_all.outliers, train_df_all.predict_outliers)
precision_score(train_df_all.outliers, train_df_all.predict_outliers)
recall_score(train_df_all.outliers, train_df_all.predict_outliers)
train_df_all.predict_proba_outliers.describe()
train_df_all.target.describe()
train_df_all.query('predict_proba_outliers > 0.53').target.describe()
train_df_all.query('predict_proba_outliers <= 0.53').target.describe()