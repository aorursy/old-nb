...
date_chunks = pd.read_csv("../input/train_date.csv", 
...
num_chunks = pd.read_csv("../input/train_numeric.csv", 
...
# iterating over chunks pull random subset (5%)

# added random seed so that analysis is repeatable

X = pd.concat([pd.concat([dchunk, nchunk], axis=1).sample(frac=0.05, random_state=1234)
...
X = np.concatenate([
    ...
    pd.read_csv("../input/train_date.csv", 
    ...
    pd.read_csv("../input/train_numeric.csv", 
